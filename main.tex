...
%		\draw[Circle - , color=mpldefaultblue] ($ (c) + (2.46,1.6) $) -- ($ (b) + (5,-.8) $);
	...
	\begin{frame}[t]{Data-Driven Methods}
		...
		\only<2>{
		\hspace{-1em} Search for suitable application-specific architecture
	...
	\begin{frame}[t]{Domain and Task}
		...
		\begin{tikzpicture}
			...
			\draw[-Stealth] (xsample) -- node[midway, above, font=\tiny]{predictive function f} (ysample);
		\end{tikzpicture}
		\vfill\tiny
