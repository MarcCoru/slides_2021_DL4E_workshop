...
%			\draw[-Stealth]($(weights.north east)+(.2,0)$) -- ($(dstest.south west)+(.2,0)$);
	...
	\begin{frame}
		\begin{tikzpicture}
			...
			\node (metaparameters) at (0,0){$\theta^\ast$};
			
			\visible<1>{
				\node[circle,outer color=white,inner color=montrose](a) at (3,2){$D_1 \iidsim P_1(\mathcal{X} \times \mathcal{Y})$};
				\node[circle,outer color=white,inner color=acier](b) at (6,0){$D_2 \iidsim P_2(\mathcal{X} \times \mathcal{Y})$};
				\node[circle,outer color=white,inner color=vertedeau](c) at (3,-2){$D_3 \iidsim P_3(\mathcal{X} \times \mathcal{Y})$};
			}
			\visible<2>{
				\node[circle,outer color=white,inner color=EPFLred](c) at (5,-2){$D \iidsim Q(\mathcal{X} \times \mathcal{Y})$};
			}
			
			\node (metaparameters) at (1,0){$\theta^\ast$};
...
%%		\begin{algorithm}[t]
%			\SetAlgoLined
%			$p(\mathcal{T})$: distribution over tasks\; $\alpha, \beta$: step size hyperparameters\;
%			randomly initialize $\theta$\;
%			\Repeat{convergence}{
%				sample batch of tasks $\tau \sim p(\mathcal{T})$\;
%				\ForEach{$\tau_{i} \in \tau$}{
%					initialize $\phi_{i}$ with $\theta$\;
%					sample $\{{D}_\text{support}, {D}_\text{query}\} \sim p(\tau_i)$\;
%					%\For{$k$ steps}{
%					evaluate $\V{g}=\nabla_{\phi_{i}}\mathcal{L}_{\tau_{i}}(f_{\phi_{i}},{D}_\text{support})$\;% with respect to $k$ examples\;
%					adapt parameters $\phi_{i} \leftarrow \phi_{i} - \alpha\V{g}$\;
%					%}
%					evaluate test loss $\mathcal{L}_{\tau_{i}}(f_{\phi_{i}},{D}_\text{query})$ \;
%				}{
%					update $\theta \leftarrow \theta - \beta \sum_{\tau_{i} \sim p(\tau)} \nabla_{\theta}\mathcal{L}_{\tau_{i}}(f_{\phi_{i}},{D}_\text{query}^{\tau_i})$\;
%				}
%			}
...
%	\end{frame}
	...
	\begin{frame}{Sen12MS-Land Cover Classification}
		\begin{tikzpicture}
			\node(a) at (0,0){\includegraphics[width=8cm]{images/sen12ms_metasplit}};
			\node[left=-1em of a]{\includegraphics[width=4cm]{images/rgb_fold_overlayed}};
		\end{tikzpicture}
	...
	\begin{frame}{2-shot 4-way Few-shot Task}
		\only<1>{\includegraphics[width=\textwidth]{images/human_few_shot1_cat0}}
		\only<2>{\includegraphics[width=\textwidth]{images/human_few_shot1_cat2}}
	\end{frame}
